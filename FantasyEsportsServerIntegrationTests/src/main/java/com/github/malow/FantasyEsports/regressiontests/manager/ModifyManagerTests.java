...
public class ModifyManagerTests extends FantasyEsportsTestFixture
{
  ...
  public void testAcceptInviteSuccessful() throws Exception
  {
    ...
    inviteManager(league.id, PRE_REGISTERED_USER1.sessionKey, PRE_REGISTERED_USER2.accountId);
    ResponseManager inviteeManager = getManagersForLeague(league.id).stream()
    ...
    inviteeManager = getManagersForLeague(league.id).stream()
        .filter(m -> m.accountId.equals(PRE_REGISTERED_USER2.accountId)).findFirst().get();
  ...
  public void testWithBadSession() throws Exception
  {
    ...
    HttpResponse<String> response = makePatchRequest("/manager/" + managerId, request, ImmutableMap.of("Session-Key", "badSession"));
  ...
  public void testWithBadManagerId() throws Exception
  {
    ...
    HttpResponse<String> response = makePatchRequest("/manager/" + "badManagerId", request,
  ...
  {
    ResponseLeague league = createLeague("Test123", PRE_REGISTERED_USER1.sessionKey);
    ResponseManager manager = getManagersForLeague(league.id).get(0);
    ModifyManagerRequest request = new ModifyManagerRequest(LeagueRole.MEMBER);

    HttpResponse<String> response = makePatchRequest("/manager/" + manager.id, request,
        ImmutableMap.of("Session-Key", PRE_REGISTERED_USER1.sessionKey));
